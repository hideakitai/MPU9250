...
#ifndef QUATERNION_FILTER
	#define QUATERNION_FILTER 1
#endif

class QuaternionFilter {
#if QUATERNION_FILTER == 1
// Madgwick accel magnetometer filter
...
public:
    ...
    void update(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz, float* q, float deltaT) {
...
#if QUATERNION_FILTER == 0
	// No filter simply integrate gyro
public:
    void bind() {}
 	void update(float ax, float ay, float az, float gx, float gy, float gz, float mx, float my, float mz, float *q, float deltaT ) {
