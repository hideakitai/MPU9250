...
class MPU9250_ {
    ...
    DriftFilter filter;		// depending on filter choice
...
public:
	...
	void start() { oldTime=newTime= micros();}
    ...
    bool update() {
		...
		if(deltaT <= (MAG_MODE==0x02? 135:12 ) ) 
		{
			// Serial.println("To fast");
			return false; // below Magnetometer sample frequency
        }
		oldTime = newTime;
        ...
        filter.update(-a[0], a[1], a[2], g[0] * DEG_TO_RAD, -g[1] * DEG_TO_RAD, -g[2] * DEG_TO_RAD, m[1], -m[0], m[2], deltaT);
        
        //filter.update(a[0], a[1], a[2], g[0] * DEG_TO_RAD, g[1] * DEG_TO_RAD, g[2] * DEG_TO_RAD, m[0], m[1], m[2], deltaT);
    ...
    float getQuaternion(const uint8_t i) const { return filter.getQuaternion(i); }
    float getQuaternionX() const { return filter.getQuaternionX(); }
    float getQuaternionY() const { return filter.getQuaternionY();}
    float getQuaternionZ() const { return filter.getQuaternionZ(); }
    float getQuaternionW() const { return filter.getQuaternionW(); }
    ...
    void setMagneticDeclination(const float d) { filter.setMagneticDeclination(d);}
    ...
    void printQuaternionHead() const {
		filter.printQuaternionHead();
    }
    void printQuaternion() const {
		filter.printQuaternion();
    }
    ...
    void printCalibration() const {
        ...
        Serial.print(acc_bias[0] * 1000.f / (float)CALIB_ACCEL_SENSITIVITY);
        Serial.print(", ");
        Serial.print(acc_bias[1] * 1000.f / (float)CALIB_ACCEL_SENSITIVITY);
        Serial.print(", ");
        Serial.print(acc_bias[2] * 1000.f / (float)CALIB_ACCEL_SENSITIVITY);
        Serial.println();
        Serial.println("gyro bias [deg/s]: ");
        Serial.print(gyro_bias[0] / (float)CALIB_GYRO_SENSITIVITY);
        Serial.print(", ");
        Serial.print(gyro_bias[1] / (float)CALIB_GYRO_SENSITIVITY);
        Serial.print(", ");
        Serial.print(gyro_bias[2] / (float)CALIB_GYRO_SENSITIVITY);
        Serial.println();
...
private:
    ...
    int16_t read_temperature_data() {
        ...
        read_bytes(MPU9250_ADDRESS, TEMP_OUT_H, 2, &raw_data[0]);  // Read the two raw data registers sequentially into data array
