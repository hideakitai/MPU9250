...
//#define DRIFT_FILTER 0   // no filter
#define DRIFT_FILTER 1   // Madgwick accel magno
//#define DRIFT_FILTER 2   // Mahony accel
#include "MPU9250.h"
...
unsigned long output_delta=50; // milisconden;
...
void setup() {
    pinMode(LED, OUTPUT);
    pinMode(LED2, OUTPUT);
    digitalWrite(LED2, HIGH);    // signal leave MPU flat
    Serial.begin(115200);
    Wire.begin();
    delay(2000);
...
void showData()
{
  ...
  {
    Serial.print("\t");
    Serial.println(mpu.getTemperature(),6);
  }
  Serial.println("");
}
